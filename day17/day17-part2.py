...
with open(sys.argv[1]) as f:
    ...
    for x, char in enumerate(line):
        cube[(x, 0, 0, 0)] = MAPPING[char]
    y = 0
    for line in f:
        y += 1
        for x, char in enumerate(line.rstrip()):
            cube[(x, y, 0, 0)] = MAPPING[char]
                ...
                for y in range(x_start - i, x_stop + i):
                    for x in range(x_start - i, x_stop + i):
                        neighbors = sum(
                            sum(
                                sum(
                                    sum(cube.get((xp, yp, zp, wp), 0) for xp in range(x - 1, x + 2))
                                    for yp in range(y - 1, y + 2)
                                ) for zp in range(z - 1, z + 2)
                            ) for wp in range(w - 1, w + 2)
                        )
                        is_active = cube.get((x, y, z, w), 0)
                        neighbors -= is_active
